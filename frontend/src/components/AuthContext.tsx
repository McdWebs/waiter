...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      const data = await apiFetch<AuthLoginRegisterResponse>('/api/auth/login', {
        method: 'POST',
  ...
  const register = useCallback(
    async (params: {
      ...
      const data = await apiFetch<AuthLoginRegisterResponse>('/api/auth/register', {
        method: 'POST',
  ...
  const value = useMemo(
    ...
    [token, owner, restaurant, loading, login, register, logout, updateRestaurant]
  )
