...
export default function CartDrawer({
  ...
  const sendCartMessage = async () => {
    ...
    const userMessage: CartChatMessage = {
      role: "user",
          ...
          {items.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center gap-3 rounded-2xl border border-dashed border-slate-200 bg-slate-50/60 px-4 py-6 text-center">
              ...
              <p className="text-[11px] text-slate-500">
                ...
                <span className="font-semibold">Add</span> to start building
                your order.
              </p>
            </div>
          ) : (
            <div className="space-y-3">
              {items.map((item) => {
                const imageUrl =
                  item.imageUrl ?? imageByMenuId.get(item.menuItemId);
                return (
                  <div
                    key={item.menuItemId}
                  ...
                  >
                    ...
                    <div className="flex flex-col items-end gap-1">
                      <div className="inline-flex items-center rounded-full border border-slate-300 bg-white">
                        ...
                        <span className="px-2 text-[11px] text-slate-900">
                          {item.quantity}
                        </span>
                        <button
                          type="button"
        ...
        <div className="border-t border-slate-200 px-4 py-3 text-xs">
          ...
          <div className="mb-2 flex flex-col gap-2">
            <MascotAssistantTrigger
              ...
              label={
                chatOpen
                  ? "Hide Servo order assistant"
                  : "Talk to Servo about this order"
              }
            />
            <button
              type="button"
          ...
          {chatOpen && (
            <div className="mt-2 rounded-2xl border border-slate-200 bg-slate-50 px-3 py-2 text-[11px] text-slate-800">
              ...
              >
                {chatMessages.length === 0 && (
                  <p className="text-[11px] text-slate-500">
                    Ask about allergens, swap a dish, or get a recommendation
