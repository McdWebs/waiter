...
async function searchPlace(query: string): Promise<LatLngLiteral & { display_name?: string } | null> {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 p-4">
      <div className="flex max-h-[90vh] w-full max-w-2xl flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xl">
        <div className="border-b border-slate-200 px-4 py-3">
          <h3 className="text-sm font-semibold text-slate-900">Pick location on map</h3>
          <p className="mt-0.5 text-xs text-slate-500">
            Search for an address or click on the map. Confirm to fill the address.
          </p>
        ...
        {error && (
          <p className="px-4 py-2 text-xs text-amber-600">{error}</p>
        )}
