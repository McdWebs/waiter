...
const SuperAdminAuthContext = createContext<SuperAdminAuthState | undefined>(
  undefined,
);
...
export function SuperAdminAuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const data = await apiFetch<SuperAdminMeResponse>(
  ...
  const loginSuperAdmin = useCallback(
    async (email: string, password: string) => {
      const data = await apiFetch<SuperAdminLoginResponse>(
