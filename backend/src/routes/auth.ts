...
router.post('/auth/register', async (req, res) => {
  try {
    const { email, password, restaurantName, restaurantSlug, currency } = req.body as {
      email?: string
      password?: string
    ...
    if (!email || !email.trim() || !password || password.length < 8 || !restaurantName?.trim()) {
      return res.status(400).json({ message: 'Email, password (min 8 chars), and restaurant name are required' })
    }
    ...
    const existingOwner = await RestaurantOwner.findOne({ email: normalizedEmail }).lean()
    if (existingOwner) {
    ...
    if (!slug) {
      return res.status(400).json({ message: 'Valid restaurant slug is required' })
    }

    const existingRestaurant = await Restaurant.findOne({ slug }).lean()
    if (existingRestaurant) {
      return res.status(409).json({ message: 'Restaurant slug is already in use' })
    }
    ...
    const token = jwt.sign(
      { ownerId: owner._id.toString(), restaurantId: restaurant._id.toString() },
      jwtSecret,
      { expiresIn: '12h' }
    )
    ...
    console.error(err)
    return res.status(500).json({ message: 'Failed to register restaurant owner' })
  }
})
...
router.post('/auth/login', async (req, res) => {
  try {
    ...
    if (!restaurant) {
      return res.status(500).json({ message: 'Associated restaurant not found' })
    }

    const token = jwt.sign(
      { ownerId: owner._id.toString(), restaurantId: owner.restaurantId.toString() },
      jwtSecret,
      { expiresIn: '12h' }
    )
