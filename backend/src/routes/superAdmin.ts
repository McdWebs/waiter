...
router.patch("/feedback/:id", async (req, res) => {
  try {
    ...
    if (Object.keys(update).length === 0) {
      return res
        .status(400)
        .json({ message: "Provide status and/or adminReply" });
    }
    ...
    const [
      ...
      WaiterCall.aggregate([
        ...
        },
        {
          $project: {
            minutes: {
              $divide: [{ $subtract: ["$handledAt", "$createdAt"] }, 60 * 1000],
            },
          },
        },
...
router.patch("/restaurants/:id", async (req, res) => {
  try {
    ...
    if (typeof body.slug === "string" && body.slug.trim()) {
      update.slug = (body.slug as string)
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/^-+|-+$/g, "");
...
router.delete("/restaurants/:id", async (req, res) => {
  try {
    ...
    const categories = await MenuCategory.find({ restaurantId: rid })
      .select({ _id: 1 })
      .lean();
    const categoryIds = categories.map((c) => c._id);

    const orderIds = (
      await Order.find({ restaurantId: rid }).select({ _id: 1 }).lean()
    ).map((o) => o._id);
